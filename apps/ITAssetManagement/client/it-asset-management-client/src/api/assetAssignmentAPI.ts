...
export const getAssignmentDetailsByCode = async (token: string, code: string) => {
  try {
    const response = await axios.get(`${BASE_URL}/Assign/GetAssignmentDetailsByCode/${encodeURIComponent(code)}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
      timeout: 5000,
